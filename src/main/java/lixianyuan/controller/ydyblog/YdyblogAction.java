...
public class YdyblogAction {
	...
	private final String key = "lixianyuan_ydy_api";

	// 测试的
	/*
	 * @RequestMapping("/hello") public String hello() throws Exception {
	 * System.out.println("进入controller拉"); return "index"; }
	 */
	...
	public @ResponseBody String addYdyblog(Ydyblog ydyblog) throws Exception {
		...
		// 如果添加成功，则删除redis中的缓存，不然显示笔记的页面的数据不会刷新
		delRedisCache_lixianyuan_ydy_api(result);
	...
	public @ResponseBody String updateYdyblog(Ydyblog ydyblog) throws Exception {
		...
		// 如果修改成功，则删除redis中的缓存，不然显示笔记的页面的数据不会刷新
		delRedisCache_lixianyuan_ydy_api(result);
	...
	public @ResponseBody String delYdyblog(String ydyblogid) throws Exception {
		...
		// 如果删除成功，则删除redis中的缓存，不然显示笔记的页面的数据不会刷新
		delRedisCache_lixianyuan_ydy_api(result);
	...
	/**
	 * 删除redis中的缓存，不然显示笔记的页面的数据不会刷新出最新的数据
	 * @param result
	 */
	public void delRedisCache_lixianyuan_ydy_api(int result) {
		if (result > 0) {
	...
	public @ResponseBody PageBean findYdyblogByCondition(String currentPage, String currentCount, String title,

...
// 按条件查询出所有博客信息
function findAllbkyblog() {

	$.ajax({
		url : contextPath + '/bky/findBkyblogByCondition.action',
		data : {
		...
		success : function(data){
			...
			if(data.pageList!=null){
				...
				for (var i = 0; i < data.pageList.length; i++) {
					var bkyblogid = data.pageList[i].bkyblogid;
					var title = data.pageList[i].title;
					var url = data.pageList[i].url;
	
					bodyContent += "<tr>";
					bodyContent += "<td>" + bkyblogid + "</td>";
					bodyContent += "<td>" + title + "</td>";
					bodyContent += "<td>" + url + "</td>";
					bodyContent += "<td>"
					bodyContent += "<button class='layui-btn layui-btn-xs' onclick='updatebkyClick(this)'><input type='hidden' value='"+bkyblogid+"'/><input type='hidden' value='"+title+"'/><input type='hidden' value='"+url+"'/>修改</button>";
					bodyContent += "<button class='layui-btn layui-btn-xs' onclick='delbkyClick(this)'><input type='hidden' value='"+bkyblogid+"'/>删除</button>";
					bodyContent += "</td>";
					bodyContent += "</tr>";
...
function addbkyClick() {
	...
	layui.form.on('submit(addFormDemo)', function(data) {
		...
		// 进入这里说明表单校验成功
		$.ajax({
			url : contextPath + '/bky/addBkyblog.action',
...
function updatebkyClick(obj) {
	...
	//初始化修改的表单
	$("#updatebkyblogId").val($(obj).children("input").eq(0).val());
	...
	layui.form.on('submit(updateFormDemo)', function(data) {
		// 进入这里说明表单校验成功
		$.ajax({
			url : contextPath + '/bky/updateBkyblog.action',
...
function delConfirmClick() {
	...
	// 进入这里说明表单校验成功
	$.ajax({
		url : contextPath + '/bky/delBkyblog.action',
		data : {"bkyblogid":$("#delydyblogid").val()},
		type : 'POST',
		async : false,
		success : function(data) {
			alert(data);
		...
		layui.laypage.render({
			...
			jump : function(obj, first) { // jump切换分页的回调
				...
				if(!first){//首次不执行(点击的时候才执行)
					//刷新数据
					findAllbkyblog();//执行查询分页函数(这个函数必须写在这里)
                }
			}
		});
	});

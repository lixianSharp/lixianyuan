...
function findAllLeavemsg() {
	...
	$.ajax({
		url : contextPath + '/leavemsg/findLeavemsgByCondition.action',
		data : {
		...
		success : function(data){
			...
			if(data.pageList!=null){
				...
				for (var i = 0; i < data.pageList.length; i++) {
					...
					bodyContent += "<td>"
					bodyContent += "<button class='layui-btn layui-btn-xs' onclick='updateYdyClick(this)'><input type='hidden' value='"+leavemsgid+"'/><input type='hidden' value='"+name+"'/><input type='hidden' value='"+email+"'/><input type='hidden' value='"+content+"'/>修改</button>";
					bodyContent += "<button class='layui-btn layui-btn-xs' onclick='delYdyClick(this)'><input type='hidden' value='"+leavemsgid+"'/>删除</button>";
					bodyContent += "</td>";
					bodyContent += "</tr>";
...
function updateYdyClick(obj) {
	...
	//初始化修改的表单
	$("#updateLsgId").val($(obj).children("input").eq(0).val());
	$("#updateName").val($(obj).children("input").eq(1).val());
	$("#updateEmail").val($(obj).children("input").eq(2).val());
	$("#updateContent").val($(obj).children("input").eq(3).val());
	...
	layui.form.on('submit(updateFormDemo)', function(data) {
		// 进入这里说明表单校验成功
		$.ajax({
			url : contextPath + '/leavemsg/updateLeavemsg.action',
...
function delConfirmClick() {
	...
	// 进入这里说明表单校验成功
	$.ajax({
		url : contextPath + '/leavemsg/delLeavemsgid.action',
		data : {"leavemsgid":$("#delydyblogid").val()},
		type : 'POST',
		async : false,
		success : function(data) {
			alert(data);

<%@ page language="java" import="java.util.*" pageEncoding="UTF-8"%>


<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN">
<html>
<head>


<title>关于我</title>
<!-- <link href="../css/about.css" rel="stylesheet" /> -->
<meta http-equiv="pragma" content="no-cache">
<meta http-equiv="cache-control" content="no-cache">
<meta http-equiv="expires" content="0">
<meta http-equiv="keywords" content="keyword1,keyword2,keyword3">
<meta http-equiv="description" content="This is my page">
<link rel="stylesheet" href="../css/normalJsp/7.css">	


</head>

<body>
	<nav class="nav">
		<ul>
			<li><a href="../index.jsp"><span>首页</span></a></li>
			<li><a href="../PathOfProcedure/index.html"><span>程序之路</span></a></li>
			<li><a href="../normalJsp/3.jsp"><span>闲言碎语</span></a></li>
			<li><a href="../normalJsp/4.jsp"><span>相册</span></a></li>
			<li><a href="../normalJsp/5.jsp"><span>学不尽</span></a></li>
			<li><a href="../normalJsp/6.jsp"><span>留言</span></a></li>
			<li><a href="../normalJsp/7.jsp"><span>关于我</span></a></li>
		</ul>
	</nav>
	<!-- <!--/nav-->
	<div class="blank"></div>

	<!-- 关于的内容 -->
	<div id="about">
		
		
		本站内容仅供学习和参阅，不做任何商业用途。<br/><br/>
		 内容如有侵犯，请立即联系我进行删除。<br/><br/>

		 个人简介<br/>
		 中文名：李贤元<br/>
		 微信公众号：李贤元个人公众号  
		 <br/> <br/> <br/><br/> 
	</div>
	<br/><br/><br/><hr/>
	<footer id="copright">Copyright &copy 2017 李贤元博客 <a href="http://www.miitbeian.gov.cn/">闽ICP备17018144号</a> </footer>
</body>
</html>
